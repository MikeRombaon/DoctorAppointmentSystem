...
public class ReportsController : ControllerBase
{
    ...
    // ── Appointment Report ────────────────────────────────────────────────────
    // GET api/reports/appointments?from=&to=&dentistId=
        ...
        [FromQuery] int? dentistId)
    {
        ...
        if (dentistId.HasValue)
            appts = appts.Where(a => a.DentistId == dentistId.Value).ToList();

        var byStatus = appts.GroupBy(a => a.Status.ToString())
            .Select(g => new { Status = g.Key, Count = g.Count() }).ToList();

        var byDentist = appts.GroupBy(a => a.DentistId)
            .Select(g => new { DentistId = g.Key, Count = g.Count() }).ToList();
    ...
    // GET api/reports/clinical/dentist-performance?from=&to=
    [HttpGet("clinical/dentist-performance")]
        ...
        [FromQuery] DateTime? from, [FromQuery] DateTime? to)
    {
        ...
        var frequency = treatments
            ...
            .Select(g =>
            {
                var proc = procedureMap.GetValueOrDefault(g.Key);
                return new
                {
                    ProcedureId = g.Key,
