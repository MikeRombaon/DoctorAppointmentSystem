...
export const reportService = {
  ...
  getAppointments: async ({ from, to, dentistId } = {}) => {
    const params = {};
    if (from) params.from = from;
    if (to) params.to = to;
  ...
  getDentistPerformance: async ({ from, to } = {}) => {
    const params = {};
    if (from) params.from = from;
    if (to) params.to = to;
    const response = await apiClient.get('/reports/clinical/dentist-performance', { params });
    return response.data;
  },
