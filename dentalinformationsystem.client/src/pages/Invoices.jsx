...
const Invoices = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        // Build invoice items from selected treatments
        const invoiceItems = selectedTreatments.map(t => ({
          treatmentId: t.id,
  ...
  // Load patient's completed unbilled treatments
  const loadPatientTreatments = async (patientId) => {
    ...
    try {
      // Fetch completed treatments for this patient
      const data = await treatmentService.getAll(1, 100, null, patientId, 'Completed');
                ...
                <Grid item xs={12}>
                  ...
                  {patientTreatments.length === 0 ? (
                    <Typography variant="body2" color="text.secondary" sx={{ py: 2 }}>
                      No completed treatments available for billing.
                    </Typography>
                  ) : (
