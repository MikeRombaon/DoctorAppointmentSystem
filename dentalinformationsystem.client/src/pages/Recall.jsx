      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box display="flex" flexDirection="column" gap={2} pt={1}>
            ...
            <TextField label="Search Patient" value={patientSearch}
              onChange={(e) => setPatientSearch(e.target.value)} size="small" />
            {patients.length > 0 && (
              <FormControl fullWidth size="small">
                <InputLabel>Select Patient</InputLabel>
                <Select value={form.patientId} label="Select Patient"
                  onChange={(e) => setForm({ ...form, patientId: e.target.value })}>
                  {patients.map((p) => (
                    <MenuItem key={p.id} value={p.id}>{p.fullName} — {p.email}</MenuItem>
                  ))}
                </Select>
              </FormControl>
            )}
