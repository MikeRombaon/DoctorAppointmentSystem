  ...
  return (
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2} flexWrap="wrap" gap={1}>
        <Box>
          <Typography variant="h5" fontWeight={700}>Doctor Scheduling</Typography>
      ...
      <Box display="flex" gap={2} mb={2} flexWrap="wrap">
        <FormControl size="small" sx={{ minWidth: 220 }}>
          ...
          <Select value={selectedDentist} onChange={e => setSelectedDentist(e.target.value)} label="Doctor">
        ...
        <DialogContent>
          <Box display="flex" flexDirection="column" gap={2} pt={1}>
            <FormControl size="small" required fullWidth>
              <InputLabel>Doctor *</InputLabel>
              <Select value={blockForm.dentistId} onChange={e => setBlockForm({ ...blockForm, dentistId: e.target.value })} label="Doctor *">
                {dentists.map(d => <MenuItem key={d.id} value={d.id}>{d.fullName || `${d.firstName} ${d.lastName}`}</MenuItem>)}
              </Select>
            </FormControl>
